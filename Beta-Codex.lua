...
-- Ensure proper parenting of ScreenGui
local success, err = pcall(function()
    screenGui.Parent = Players.LocalPlayer:WaitForChild("PlayerGui")
end)

if not success then
    debugPrint("Failed to parent ScreenGui: " .. tostring(err))
    return
end

-- Check if screenGui is initialized
if not screenGui then 
    debugPrint("screenGui is not initialized")
    return
end
...
-- Modern Health/Energy Bar System
local function createModernStatusBars()
    ...
    local function setupBars()
        ...
        -- Remove existing status bars if they exist
        local existingBars = screenGui:FindFirstChild("ModernStatusBars")
        if existingBars then
            existingBars:Destroy()
        end

        -- Container for both bars
        local statusContainer = Instance.new("Frame")
        statusContainer.Name = "ModernStatusBars"
        statusContainer.Size = UDim2.new(0, 400, 0, 50)
        statusContainer.Position = UDim2.new(0.5, -200, 1, -70)  -- Bottom center
        statusContainer.BackgroundTransparency = 1
        statusContainer.Parent = screenGui

        -- Create a single status bar (health or energy)
        local function createStatusBar(name, color, yOffset)
            local barContainer = Instance.new("Frame")
            barContainer.Name = name .. "Container"
            barContainer.Size = UDim2.new(1, 0, 0, 20)
            barContainer.Position = UDim2.new(0, 0, 0, yOffset)
            barContainer.BackgroundColor3 = Color3.fromRGB(27, 32, 48)
            barContainer.BackgroundTransparency = 0.1
            barContainer.Parent = statusContainer
            ...
            -- Stroke effect
            local stroke = Instance.new("UIStroke")
            stroke.Color = Color3.fromRGB(40, 65, 120)
            stroke.Thickness = 1
            stroke.Transparency = 0.7
            stroke.Parent = barContainer

            -- Fill bar
            local fillBar = Instance.new("Frame")
            fillBar.Name = name .. "Fill"
        ...
        local healthBar = createStatusBar("Health", Color3.fromRGB(88, 207, 144), 0)  -- Green
        local energyBar = createStatusBar("Energy", Color3.fromRGB(82, 176, 255), 25) -- Blue
        ...
        local function updateBars()
            if humanoid then
                ...
                TweenService:Create(healthBar.fill, 
                    TweenInfo.new(0.3, Enum.EasingStyle.Quad), 
                    {Size = UDim2.new(healthPercent, 0, 1, 0)}
                ):Play()
                
                healthBar.value.Text = string.format("Health: %d/%d", math.floor(health), math.floor(maxHealth))

                -- Critical health effect
                if healthPercent < 0.3 then
                    TweenService:Create(healthBar.glow,
                        TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, -1, true),
                        {ImageTransparency = 0.6}
                    ):Play()
                else
                    healthBar.glow.ImageTransparency = 0.8
                end
            end

            if stats then
                local energy = stats:FindFirstChild("Energy") and stats.Energy.Value or 0
                local maxEnergy = stats:FindFirstChild("MaxEnergy") and stats.MaxEnergy.Value or 100
                local energyPercent = math.clamp(energy / maxEnergy, 0, 1)
                
                -- Smooth energy bar update
                TweenService:Create(energyBar.fill,
                    TweenInfo.new(0.3, Enum.EasingStyle.Quad),
                    {Size = UDim2.new(energyPercent, 0, 1, 0)}
                ):Play()
                
                energyBar.value.Text = string.format("Energy: %d/%d", energy, maxEnergy)

                -- Low energy effect
                if energyPercent < 0.3 then
                    TweenService:Create(energyBar.glow,
                        TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, -1, true),
                        {ImageTransparency = 0.6}
                    ):Play()
                else
                    energyBar.glow.ImageTransparency = 0.8
                end
            end
        end
    ...
    -- Set up bars when character loads
    local success, err = pcall(function()
        setupBars()
        player.CharacterAdded:Connect(setupBars)
    end)

    if not success then
        debugPrint("Failed to setup status bars: " .. tostring(err))
    end
end

-- Initialize everything in the correct order
local function init()
    -- Create main UI first
    [Your existing showButtonList() and other UI initialization]

    -- Create status bars last
    spawn(function()
        wait(1)  -- Give time for other elements to load
        createModernStatusBars()
    end)
end
